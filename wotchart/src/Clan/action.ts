...
export const Actions = {
  ...
  fetchhPlayerPersonalDataSucceded: (playerDetails: PlayerDetails[]) => createAction(ActionTypes.FETCH_PLAYER_PERSONAL_DATA_SUCCEDED, playerDetails),
};
...
export function fetchhPlayerPersonalDataRequested(options: FetchPlayerPersonalDataOptions): DispatchAction {
  return async dispatch => {
    const requestOptions = {
      method: 'POST',
      body: `application_id=${applicationId}&account_id:${options.accountIds.join(',')}`,
    }

    fetch(`https://api.worldoftanks.asia/wot/account/info/`, requestOptions)
      .then(response => response.json())
      .then(data => dispatch(Actions.fetchhPlayerPersonalDataSucceded(data.data)));
  };
}
